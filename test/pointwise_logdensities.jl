@testset "logdensities_likelihoods.jl" begin
    ...
    @testset "$(model.f)" for model in DynamicPPL.DemoModels.DEMO_MODELS
        ...
        loglikelihood_true = DynamicPPL.DemoModels.loglikelihood_true(
...
@testset "pointwise_logdensities chain" begin
    ...
    model = DynamicPPL.DemoModels.demo_dot_assume_dot_observe()
    ...
    num_iters = 3
    vals = [DynamicPPL.DemoModels.rand_prior_true(model) for _ in 1:num_iters]
    ...
    for (val, logjoint, logprior, loglikelihood) in
        ...
        logjoint_true = DynamicPPL.DemoModels.logjoint_true(model, val...)
        logprior_true = DynamicPPL.DemoModels.logprior_true(model, val...)
        loglikelihood_true = DynamicPPL.DemoModels.loglikelihood_true(model, val...)
