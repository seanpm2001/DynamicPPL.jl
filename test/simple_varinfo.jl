@testset "simple_varinfo.jl" begin
    ...
    @testset "link!! & invlink!! on $(nameof(model))" for model in
        ...
        values_constrained = DynamicPPL.DemoModels.rand_prior_true(model)
        @testset "$(typeof(vi))" for vi in (
            ...
            values_unconstrained, lp_linked_true = DynamicPPL.DemoModels.logjoint_true_with_logabsdet_jacobian(
            ...
            lp_invlinked_true = DynamicPPL.DemoModels.logjoint_true(
                model, values_constrained...
            )
            # Should result in the correct logjoint.
            ...
            @test all(
                ...
                vn in DynamicPPL.DemoModels.varnames(model)
            )
        end
    end
    ...
    @testset "SimpleVarInfo on $(nameof(model))" for model in
                                                     DynamicPPL.DemoModels.DEMO_MODELS
        model = DynamicPPL.DemoModels.demo_dot_assume_matrix_dot_observe_matrix()

        # We might need to pre-allocate for the variable `m`, so we need
        # to see whether this is the case.
        svi_nt = SimpleVarInfo(DynamicPPL.DemoModels.rand_prior_true(model))
        svi_dict = SimpleVarInfo(VarInfo(model), Dict)
        vnv = DynamicPPL.VarNamedVector()
        for (k, v) in pairs(DynamicPPL.DemoModels.rand_prior_true(model))
        ...
        @testset "$(nameof(typeof(DynamicPPL.values_as(svi))))" for svi in (
            ...
            ### Evaluation ###
            values_eval_constrained = DynamicPPL.DemoModels.rand_prior_true(model)
            if DynamicPPL.istrans(svi)
                _values_prior, logpri_true = DynamicPPL.DemoModels.logprior_true_with_logabsdet_jacobian(
                    model, values_eval_constrained...
                )
                values_eval, logπ_true = DynamicPPL.DemoModels.logjoint_true_with_logabsdet_jacobian(
                    model, values_eval_constrained...
                )
                ...
                logpri_true = DynamicPPL.DemoModels.logprior_true(
                    model, values_eval_constrained...
                )
                logπ_true = DynamicPPL.DemoModels.logjoint_true(
                    model, values_eval_constrained...
                )
            ...
            loglik_true = DynamicPPL.DemoModels.loglikelihood_true(
                model, values_eval_constrained...
            )
    ...
    @testset "Dynamic constraints" begin
        ...
        for svi in (svi_nt, svi_vnv)
            ...
            for i in 1:10
                ...
                retval_unconstrained, lp_true = DynamicPPL.DemoModels.logjoint_true_with_logabsdet_jacobian(
    ...
    @testset "Static transformation" begin
        model = DynamicPPL.DemoModels.demo_static_transformation()

        varinfos = DynamicPPL.TestUtils.setup_varinfos(
            model, DynamicPPL.DemoModels.rand_prior_true(model), [@varname(s), @varname(m)]
        )
